...
export class AndroidRobot implements Robot {
	...
	// Map of special characters to their Android keyevent sequences
	// Used as fallback when shell escaping is unreliable
	private typeViaKeyEvents(text: string): void {
		for (const char of text) {
			const code = char.charCodeAt(0);
			// Digits 0-9: KEYCODE_0 (7) through KEYCODE_9 (16)
			if (code >= 48 && code <= 57) {
				this.adb("shell", "input", "keyevent", `${code - 48 + 7}`);
			// Lowercase a-z: KEYCODE_A (29) through KEYCODE_Z (54)
			} else if (code >= 97 && code <= 122) {
				this.adb("shell", "input", "keyevent", `${code - 97 + 29}`);
			// Uppercase A-Z: shift + keycode
			} else if (code >= 65 && code <= 90) {
				this.adb("shell", "input", "keyevent", "--longpress", "KEYCODE_SHIFT_LEFT", `${code - 65 + 29}`);
			// Space
			} else if (code === 32) {
				this.adb("shell", "input", "keyevent", "62");
			// For everything else, use input text with the single char wrapped in quotes
			} else {
				try {
					this.adb("shell", "input", "text", `'${char}'`);
				} catch {
					// If even that fails, try keyevent for common symbols
					const symbolMap: Record<string, string> = {
						"!": "KEYCODE_SHIFT_LEFT KEYCODE_1",
						"@": "KEYCODE_SHIFT_LEFT KEYCODE_2",
						"#": "KEYCODE_SHIFT_LEFT KEYCODE_3",
						"$": "KEYCODE_SHIFT_LEFT KEYCODE_4",
						"%": "KEYCODE_SHIFT_LEFT KEYCODE_5",
						"^": "KEYCODE_SHIFT_LEFT KEYCODE_6",
						"&": "KEYCODE_SHIFT_LEFT KEYCODE_7",
						"*": "KEYCODE_SHIFT_LEFT KEYCODE_8",
						"(": "KEYCODE_SHIFT_LEFT KEYCODE_9",
						")": "KEYCODE_SHIFT_LEFT KEYCODE_0",
						"-": "69",
						"=": "70",
						"[": "71",
						"]": "72",
					...
					const keys = symbolMap[char];
					if (keys) {
						if (keys.includes(" ")) {
							const parts = keys.split(" ");
							this.adb("shell", "input", "keyevent", "--longpress", ...parts);
						} else {
							this.adb("shell", "input", "keyevent", keys);
						}
					}
				}
			}
	...
	public async sendKeys(text: string): Promise<void> {
		...
		if (this.isAscii(text)) {
			// Try shell input first, fall back to keyevent for special chars
			const hasShellSpecial = /[!#`$\\'"~|&;()<>{}[\]*?]/.test(text);
			if (hasShellSpecial) {
				this.typeViaKeyEvents(text);
			} else {

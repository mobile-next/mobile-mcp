...
const createMcpServer = () => {
    ...
    const executeAction = async (robot, action) => {
        ...
        switch (cmd) {
            ...
            case "screenshot": {
                ...
                node_fs_1.default.writeFileSync(arg, screenshotData);
    ...
    tool("mobile_do", "Do", "All-in-one mobile tool. Performs action(s) then returns screen elements. Without actions, just reads screen. Each element has a stable ID like BUTTON:Confirm or INPUT:amount. Duplicates get @N suffix (BUTTON:Confirm@2). Actions: 'tap BUTTON:Confirm', 'type hello', 'press BACK', 'swipe up', 'dismiss', 'screenshot /path/to/file.png', 'wait 1000'. Use screenshot mid-batch to capture each screen during DFS exploration.", {
        device: zod_1.z.string().describe("The device identifier to use."),
        actions: zod_1.z.string().optional().describe('Action(s) before reading screen. Single string or JSON array. Examples: "tap BUTTON:Confirm", or ["tap TEXT:Send", "wait 1000", "screenshot /tmp/send.png", "press BACK"]'),

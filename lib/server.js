...
const createMcpServer = () => {
    ...
    tool("mobile_do", "Do", "All-in-one mobile tool. Performs action(s) then returns screen elements. Without actions, just reads screen. Each element has a stable ID like BUTTON:Confirm or INPUT:amount. Duplicates get @N suffix (BUTTON:Confirm@2). To tap, use the exact ID from the output: 'tap BUTTON:Confirm'. Other actions: 'type hello', 'press BACK', 'swipe up', 'wait 1000'. Tap does a fresh UI dump so keyboard/modal coordinate shifts are handled automatically.", {

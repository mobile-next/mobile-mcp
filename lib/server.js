...
const createMcpServer = () => {
    ...
    const buildElementIds = (elements) => {
    ...
    tool("mobile_do", "Do", "All-in-one mobile tool. Performs action(s) then returns screen elements. Without actions, just reads screen. Each element has a stable ID like BUTTON:Confirm or INPUT:amount. Duplicates get @N suffix (BUTTON:Confirm@2). To tap, use the exact ID from the output: 'tap BUTTON:Confirm'. Other actions: 'type hello', 'press BACK', 'swipe up', 'dismiss' (close bottom sheet by dragging handle down), 'wait 1000'. Tap does a fresh UI dump so keyboard/modal coordinate shifts are handled automatically.", {
        device: zod_1.z.string().describe("The device identifier to use."),
        actions: zod_1.z.string().optional().describe('Action(s) before reading screen. Single string or JSON array. Tap uses element IDs from output: "tap BUTTON:Confirm", or ["tap BUTTON:POL", "type 5", "wait 3000", "tap BUTTON:Confirm@2"]'),

...
const createMcpServer = () => {
    ...
    tool("mobile_batch_taps", "Batch Taps", "Execute multiple taps in rapid sequence with optional delays between them. Much faster than individual click calls. Each tap is an object with x, y coordinates and an optional delayAfter in milliseconds (default 200ms).", {
        device: zod_1.z.string().describe("The device identifier to use."),
        taps: zod_1.z.string().describe('JSON array of taps, e.g. [{"x":180,"y":1743},{"x":540,"y":1743,"delayAfter":500}]. Each tap has x, y (required) and delayAfter in ms (optional, default 200).'),
    }, { destructiveHint: true }, async ({ device, taps }) => {
        const robot = getRobotFromDevice(device);
        const tapList = JSON.parse(taps);

...
const createMcpServer = () => {
    ...
    const getNumberedElements = async (robot) => {
    ...
    tool("mobile_tap_element", "Tap Element", "Tap any element by its [#N] number from mobile_list_elements_on_screen. Performs a fresh UI dump to get current coordinates, so keyboard/modal shifts are handled automatically.", {
    ...
    tool("mobile_list_elements_on_screen", "List Screen Elements", "List all elements on screen. Every element gets a [#N] number usable with mobile_tap_element. Do not cache this result.", {
        device: zod_1.z.string().describe("The device identifier to use. Use mobile_list_available_devices to find which devices are available to you.")
    }, { readOnlyHint: true }, async ({ device }) => {
        const robot = getRobotFromDevice(device);
        const { numbered } = await getNumberedElements(robot);
